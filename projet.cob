...
AJOUTER_ENQUETE.
    ...
    ACCEPT Wfe_matrChef
    DISPLAY "Entrez la date de début d'enquete : "
    ACCEPT Wfe_dateDeb

    *> Attention il faut parcourir farchives et fenquetes pour créer l id
    MOVE 0 TO Wnb
    MOVE 0 TO Wfin
    OPEN INPUT farchives
    ...
    WRITE enqTampon
       	INVALID KEY
        	DISPLAY "Existe deja"
	    NOT INVALID KEY
	        DISPLAY "Ajout réussi. L'enquête porte le numéro "Wnb
	END-WRITE.
    CLOSE fenquetes
...
MODIFIER_ENQUETE.
	DISPLAY "Donner le numéro de l'enquête à modifier"
	ACCEPT Wfe_idEnq

	OPEN I-O fenquetes
	READ fenquetes
	INVALID KEY
		DISPLAY "Enquête introuvable."
	NOT INVALID KEY
	    DISPLAY "Modifier la date de l'enquête ? 1:OUI, 0:NON"
	    ACCEPT Wdecision
	    IF Wdecision = 1
	    THEN
	    	...
	    	ACCEPT Wfe_dateDeb
	    END-IF
	    DISPLAY "Modifier le chef d'enquête ? 1:OUI, 0:NON"
	    ACCEPT Wdecision
	    IF Wdecision = 1
	    THEN
...
SUPPRIMER_ENQUETE. *> à refaire
	DISPLAY "Donner le numéro de l'enqête à archiver"
	ACCEPT Wfe_idEnq
...
RECHERCHER_ENQUETE.
	DISPLAY "Donner l'enquête que vous voulez rechercher"
	ACCEPT Wfe_idEnq
	...
	INVALID KEY
		DISPLAY "Enquête introuvable."
	NOT INVALID KEY
		DISPLAY "Enquête existe."
	END-READ
	CLOSE fenquetes
.

AJOUTER_CRIME.
	DISPLAY "Donner le lieu du délit"
	ACCEPT Wfc_lieu
	DISPLAY "Description : "
	ACCEPT Wfc_desc
    ...
    PERFORM WITH TEST AFTER UNTIL Wdecision = 1
    	DISPLAY "Ajouter une pièce à conviction ? 1:OUI, 0:NON"
    	ACCEPT Wdecision
